...
export default class PackageController {
    ...
    try {
      ...
      if (packageFound) {
        return res
          .status(400)
          .json({
            message: `Package "${title}" already exists and is active.`,
          });
      }
      ...
      if (!provider) {
        return res
          .status(400)
          .json({
            message: "No active provider found to associate with the package.",
          });
      }
      ...
      return res
        .status(201)
        .json({ message: `Package "${title}" created successfully.` });
    } catch (error) {
      return res.status(500).json({ message: "Internal server error" });
    }
  }
  ...
  static async updatePackage(req: Request, res: Response): Promise<any> {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: `Package "${title}" updated successfully.` });
    } catch (error) {
      return res.status(500).json({ message: "Internal server error" });
    }
  }
  ...
  static async deletePackage(req: Request, res: Response): Promise<any> {
    ...
    try {
      ...
      return res
        .status(204)
        .send()
        .json({ message: `Package "${title}" deleted successfully` });
    } catch (error) {
      return res.status(500).json({ message: "Internal server error" });
    }
  }
