    ...
    try {
      ...
      if (roleFounded) {
        return res
          .status(400)
          .json({
            message: `Role "${roleName}" already exists and is active.`,
          });
      }
      ...
      return res
        .status(201)
        .json({ message: `Role "${roleName}" created successfully.` });
    } catch (error) {
  ...
  static async updateRole(req: Request, res: Response): Promise<any> {
    ...
    try {
      ...
      if (!roleFounded) {
        return res
          .status(404)
          .json({ message: `Role "${roleName}" not found or inactive.` });
      }
    ...
    try {
      ...
      if (!roleFounded) {
        return res
          .status(404)
          .json({
            message: `Role "${roleName}" not found or already inactive.`,
          });
      }
  ...
  static async getRoleById(req: Request, res: Response): Promise<any> {
    try {
      ...
      if (!role) {
        return res
          .status(404)
          .json({ message: `Role with ID "${roleId}" not found or inactive.` });
      }
