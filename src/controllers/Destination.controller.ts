...
export default class DestinationController {
    ...
    static async addNewDestination(req: Request, res: Response): Promise<any> {
        try {
            const { nameDes, city, country, description } = req.body as AddDestinationDTO;

            const DestinationFound = await prisma.destination.findFirst({
                where: { nameDes, active: true },
            });
            if (DestinationFound) {
                return res.status(400).json({ message: `Destination "${nameDes}" already exists and is active.` });
            }
            ...
            return res.status(201).json({ message: `Destination "${nameDes}" created successfully.` });
        }
        catch (error) {
            return res.status(500).json({ message: "Internal server error" });
        }
    ...
    static async updateDestination(req: Request, res: Response): Promise<any> {
        try {
            ...
            if (!destinationFounded) {
                return res.status(404).json({ message: `Destination "${nameDes}" not found or inactive.` });
            }
            ...
            return res.status(200).json({ message: `Destination "${nameDes}" updated successfully.` });
        }
        catch (error) {
            return res.status(500).json({ message: "Internal server error" });
        }
    ...
    static async deleteDestination(req: Request, res: Response): Promise<any> {
        try {
            ...
            if (!destinationFounded) {
                return res.status(404).json({ message: `Destination "${nameDes}" not found or already inactive.` });
            }
            ...
            return res.status(204).json({ message: `Destination "${nameDes}" deleted successfully.` });
