...
export default class ReviewController {
  ...
  static async addNewReview(req: Request, res: Response): Promise<any> {
    try {
      ...
      const packagee = await prisma.package
        .findFirst({
          where: { active: true },
      ...
      if (!packagee || !customer) {
        return res
          .status(400)
          .json({ message: "Not founded package o customer active." });
      }
      ...
      return res
        .status(201)
        .json({ message: `Review "${comment}" created successfully.` });
    } catch (error) {
      return res.status(500).json({ message: "Internal server error" });
    }
  }
  ...
  static async updateReview(req: Request, res: Response): Promise<any> {
    try {
      ...
      return res
        .status(200)
        .json({ message: `Review "${comment}" updated successfully.` });
    } catch (error) {
      return res.status(500).json({ message: "Internal server error" });
    }
  }
  ...
  static async deleteReview(req: Request, res: Response): Promise<any> {
    try {
      ...
      return res
        .status(204)
        .json({ message: `Review "${comment}" deleted successfully.` });
    } catch (error) {
      return res.status(500).json({ message: "Internal server error" });
    }
  }
